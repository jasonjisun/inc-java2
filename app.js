...
function validatePassword(password) {
    ...
    // Check for only integers using regular expression
    if (/^\d+$/.test(password)) {
      return false;
    }
...
regForm.addEventListener('submit', function (e) {
    ...
    if (usernameReg.value.length == 0 || passwordReg.value.length == 0) {
      ...
      if (!validatePassword(passwordReg.value)) {
        alert("Password must be at least 8 characters, contain a mix of uppercase and lowercase letters, and not consist only of numbers.");
        return; // Prevent further processing if password is invalid
      }
  
      // Rest of the existing logic for checking username existence and storing data...
    }
  });